...
# Visualize number of infected birds for each island ----
ggplot(data = infection_data, aes(x = Island, y = infection_binary)) +
  geom_col()

# what about the infection rate on each island? because I coded 1 = positive and 0 = negative, I can take the mean of 'infection_binary' for each island and get the proportion of infected individuals in the entire sample for that island
mean.infection <- infection_data %>% group_by(Island) %>% 
  summarize(infection.rate = mean(infection_binary))

# how does this compare to the visualization of total infections?
ggplot(data = mean.infection, aes(x = Island, y = infection.rate)) +
  geom_col()
# slightly different but not much 
# on Hawaii, over than 35% birds infected
# on Kauai, just over 20% infected
# on Mauai, less than 10% infected

# does Island serve as a predictor of infection status?
infect.model <- lm(formula = infection_binary ~ factor(Island), 
                   data = infection_data)
summary(infect.model)
# significant effect of infection rate for Kauai but the model is a very poor fit
